...
class Model(nn.Module):
    ...
    def imputation(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask):
        ...
        enc_out = self.enc_embedding(x_enc, x_mark_enc)
        enc_out, attns = self.encoder(enc_out, attn_mask=None)
        # Normalization from Non-stationary Transformer for d_model output
        ###########这里的逻辑感觉有问题
        dec_out_d_model = enc_out
        means_d_model = dec_out_d_model.mean(1, keepdim=True).detach()
        stdev = torch.sqrt(torch.var(dec_out_d_model, dim=1, keepdim=True, unbiased=False) + 1e-5)
        
        _, L_d_model, N_d_model = dec_out_d_model.shape

        dec_out_d_model = self.projection_remain(dec_out_d_model).permute(0, 2, 1)[:, :, :N_d_model]
        # De-Normalization from Non-stationary Transformer
        ...
        return dec_out_d_model, dec_out_time
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        if self.task_name == 'imputation':
            dec_out_d_model, dec_out_time = self.imputation(x_enc, x_mark_enc, x_dec, x_mark_dec, mask)
