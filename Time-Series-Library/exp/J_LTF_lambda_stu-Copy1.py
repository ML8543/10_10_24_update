...
class joint_loss(nn.Module):
    def __init__(self, lambda_param, imp_ls_fn, ds_ls_fn):
    ...
    def forward(self, x_imp, y_imp, x_ds, y_ds):
        ...
        total_loss = self.lambda_param * imp_loss + ds_loss
...
class Exp_Long_Term_Forecast_Imp_J_Lambda_Stu(Exp_Basic):
    ...
    def _build_lambda(self):
        _lambda = torch.FloatTensor([self.args._lambda]).to(self.device).clamp(0, 1)
        _lambda = nn.Parameter(_lambda, requires_grad=True)  # 确保设置 requires_grad 为 True
        return _lambda
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            # 在每个 epoch 结束后应用 torch.clamp 来约束 _lambda 的值
            self._lambda.data = torch.clamp(self._lambda.data, min=0, max=1)
