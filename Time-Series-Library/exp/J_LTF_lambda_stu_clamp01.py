...
# 联合训练
class Exp_Long_Term_Forecast_Imp_J_Lambda_Stu_clamp(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_Imp_J_Lambda_Stu_clamp, self).__init__(args)
        self.args = args
    ...
    def _bulid_imputation_model(self):
        ...
        imp_args.n_heads = 8
        ###Crossformer的ETTh1数据填补的下面三个参数和其他的填补模型不一样
            ...
            for i, (batch_x_raw, batch_y_raw, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        outputs = outputs[:, -self.args.pred_len:, f_dim:]
                        batch_y_raw = batch_y_raw[:, -self.args.pred_len:, f_dim:]
                        ####################*********limitation_og_laambda_changed1
                    ...
                    outputs = outputs[:, -self.args.pred_len:, f_dim:]
                    batch_y_raw = batch_y_raw[:, -self.args.pred_len:, f_dim:]
                    ####################*********limitation_og_laambda_changed2
                    self._lambda.data = torch.clamp(self._lambda.data, min=0, max=1)
                    loss, imp_loss, ds_loss = criterion(x_imp[mask==0],batch_x_raw[mask==0],outputs, batch_y_raw)
                    train_loss.append(loss.item())
